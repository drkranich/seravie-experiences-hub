...
export function SignDocument() {
  ...
  const submit = async () => {
    ...
    const { data, error } = await supabase.functions.invoke('sign-doc', { body: { token, signed_name: name.trim(), signature_data: sig } })
    setSubmitting(false)
  ...
  const errMsg = { name_required: 'Digite seu nome completo.', signature_required: 'Desenhe sua assinatura.', accept_required: 'Marque o aceite para prosseguir.', invalid_token: 'Link inválido ou expirado.', already_signed: 'Este documento já foi assinado.', cancelled: 'Esta solicitação foi cancelada.', network: 'Falha de conexão. Tente novamente.' }[err] || (err ? 'Ocorreu um erro. Tente novamente.' : '')
