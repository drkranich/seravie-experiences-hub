  ...
  const NavItem = ({ n }) => (
    <button
      ...
      className={`relative w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-[13px] transition-all duration-200 ${
        active === n.key
          ? 'bg-white/[0.06] text-admin-champ'
          : 'text-admin-muted hover:text-admin-text hover:bg-white/[0.03]'
      }`}
    >
      {active === n.key && (
        <span className="absolute left-0 top-1/2 -translate-y-1/2 h-5 w-[2px] rounded-full bg-admin-champ" />
      )}
    ...
    <div className="min-h-screen admin-bg text-admin-text flex" data-no-translate>
      ...
      <aside className="hidden lg:flex flex-col w-60 shrink-0 bg-admin-side/80 backdrop-blur-2xl border-r border-white/[0.06] sticky top-0 h-screen">
        ...
        <div className="px-5 pt-7 pb-5 border-b border-white/[0.06]">
          ...
          <div className="text-[8px] tracking-[0.2em] text-admin-champ/60 mt-1 uppercase">
            {profile?.tenant_name || 'Experiences'} · CMS
          </div>
        </div>
          ...
          {NAV_GROUPS.map((g) => (
            <div key={g.group}>
              ...
              <div className="space-y-0.5">
                {g.items.map((n) => <NavItem key={n.key} n={n} />)}
              </div>
            </div>
          ))}
        </nav>
      ...
      <div className="flex-1 min-w-0 flex flex-col">
        ...
        <header className="sticky top-0 z-30 bg-admin-side/40 backdrop-blur-xl border-b border-white/[0.06] px-6 py-3.5 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            {/* Menu mobile */}
            <button
              className="lg:hidden text-admin-muted hover:text-admin-text transition-colors"
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={onExit}
              className="inline-flex items-center gap-2 px-3 py-1.5 text-[10px] tracking-wider uppercase border border-admin-champ/20 text-admin-champ/70 rounded-lg hover:bg-white/[0.04] transition-colors"
            >
            ...
            <button
              onClick={logout}
              className="inline-flex items-center gap-2 px-3 py-1.5 text-[10px] tracking-wider uppercase text-admin-muted/60 hover:text-admin-rose transition-colors rounded-lg hover:bg-white/[0.03]"
            >
                  ...
                  {g.items.map((n) => (
                    <button
                      ...
                      className={`flex items-center gap-2 px-3 py-2 rounded-lg text-[12px] transition-colors ${
                        active === n.key
                          ? 'bg-white/[0.08] text-admin-champ'
                          : 'text-admin-muted hover:text-admin-text hover:bg-white/[0.03]'
                      }`}
                    >
      ...
      <div className="fixed bottom-6 right-6 z-50 space-y-3 pointer-events-none">
        {toasts.map((t) => (
          <div
            key={t.id}
            className={`glass rounded-xl px-5 py-3 text-[13px] flex items-center gap-3 pointer-events-auto shadow-xl ${
