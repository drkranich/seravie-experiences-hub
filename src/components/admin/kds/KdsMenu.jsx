...
export function KdsMenu({ kind = 'kitchen', notify }) {
  ...
  const filtered = useMemo(() => {
    ...
    return q ? rows.filter((r) => `${r.name} ${r.category}`.toLowerCase().includes(q)) : rows
  }, [rows, search])
  const categories = useMemo(() => [...new Set(rows.map((r) => r.category).filter(Boolean))], [rows])

  const openNew = () => setModal({ name: '', category: '', station_id: '', prep_min: 10, active: true, kind })
  const save = async () => {
    if (!modal.name.trim()) return notify?.('Informe o nome', 'error')
    const payload = { name: modal.name.trim(), category: modal.category?.trim() || null, station_id: modal.station_id || null, prep_seconds: (Number(modal.prep_min) || 10) * 60, active: modal.active, kind, tenant_id: tenantId }
          ...
          <div key={r.id} className={`glass rounded-2xl p-4 group relative ${r.active ? '' : 'opacity-60'}`}>
            <div className="flex items-start justify-between gap-2">
              <div className="min-w-0">
                ...
                <p className="text-admin-muted/50 text-[11px]">{r.category || 'sem categoria'}</p>
              </div>
            ...
            <div className="flex items-center gap-3 mt-3 text-[11px] text-admin-muted/60">
            ...
            <div className="flex gap-1 mt-3 pt-3 border-t border-white/[0.05] opacity-0 group-hover:opacity-100 transition-opacity">
              <button onClick={() => setModal({ ...r, prep_min: Math.round((r.prep_seconds || 600) / 60) })} className="p-1.5 rounded-lg text-admin-muted hover:text-admin-champ" title="Editar"><Icon name="pen" className="w-3.5 h-3.5" /></button>
