...
export function Collections({ me, notify, onOpenSupplier }) {
  ...
  const removeCollection = async (c) => {
    await supabase.from('network_collection_items').delete().eq('collection_id', c.id)
    const { error } = await supabase.from('network_collections').delete().eq('id', c.id)
    if (error) return notify?.('Erro ao excluir: ' + error.message, 'error')
    setCols((cs) => cs.filter((x) => x.id !== c.id)); setOpen(null); notify?.('Coleção excluída', 'success')
  }

  if (open) return <CollectionDetail collection={cols.find((c) => c.id === open.id) || open} tenantId={tenantId} onBack={() => { setOpen(null); load() }} onOpenSupplier={onOpenSupplier} onDelete={removeCollection} onPatch={(u) => setCols((cs) => cs.map((x) => x.id === u.id ? { ...x, ...u } : x))} notify={notify} />
...
function CreateCollection({ onClose, onCreate, notify }) {
  const [f, setF] = useState({ title: '', subtitle: '', theme: '', description: '', curator: '', cover_url: '', style: '', location: '', budget_hint: '', tags: [] })
    ...
    <div className="fixed inset-0 z-[90] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={onClose}>
      <div className="glass-pop rounded-2xl p-6 w-full max-w-lg max-h-[92vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="space-y-3">
          ...
          <div><label className={lbl}>Subtítulo / chamada</label><input value={f.subtitle} onChange={(e) => set('subtitle', e.target.value)} placeholder="Uma frase que resume o conceito" className={cls} /></div>
          <div className="grid grid-cols-2 gap-3">
            ...
            <div><label className={lbl}>Estilo</label><GlassSelect value={f.style} onChange={(v) => set('style', v)} options={[{ value: '', label: '—' }, ...STYLES.map((s) => ({ value: s, label: s }))]} /></div>
          </div>
          <div><label className={lbl}>Descrição</label><textarea value={f.description} onChange={(e) => set('description', e.target.value)} rows={3} placeholder="Conceito, atmosfera e intenção da coleção…" className={`${cls} resize-none`} /></div>
          <div className="grid grid-cols-2 gap-3">
            <div><label className={lbl}>Localização / referência</label><input value={f.location} onChange={(e) => set('location', e.target.value)} placeholder="Cidade, estilo regional…" className={cls} /></div>
            <div><label className={lbl}>Faixa de investimento</label><input value={f.budget_hint} onChange={(e) => set('budget_hint', e.target.value)} placeholder="Ex.: R$ 40–80 mil" className={cls} /></div>
          </div>
          <div><label className={lbl}>Tags (separe por vírgula)</label><input value={(f.tags || []).join(', ')} onChange={(e) => set('tags', e.target.value.split(',').map((x) => x.trim()).filter(Boolean))} placeholder="Ex.: madeira, plantas, tons terrosos" className={cls} /></div>
...
function CollectionDetail({ collection: c, tenantId, onBack, onOpenSupplier, onDelete, onPatch, notify }) {
  ...
  const share = async () => {
    ...
    if (!token || !c.is_public) {
      token = 'c' + Math.abs(c.id.split('').reduce((a, ch) => a * 31 + ch.charCodeAt(0), 7)).toString(36) + c.id.slice(0, 8)
      await supabase.from('network_collections').update({ share_token: token, is_public: true }).eq('id', c.id)
      c.share_token = token; c.is_public = true; onPatch?.({ id: c.id, share_token: token, is_public: true })
    }
    const url = `${window.location.origin}/colecao/${token}`
    try { await navigator.clipboard.writeText(url); notify?.('Link da coleção copiado!', 'success') } catch { notify?.('Link: ' + url, 'info') }
  }
    ...
    <div>
      ...
      <div className="flex items-start justify-between mb-5 flex-wrap gap-3">
        <div className="min-w-0">
          <div className="flex items-center gap-2 flex-wrap"><h1 className="font-serif text-2xl text-admin-text">{c.title}</h1>{c.theme && <span className="text-[11px] px-2 py-0.5 rounded-lg bg-white/[0.05] text-admin-muted/60">{c.theme}</span>}{c.style && <span className="text-[11px] px-2 py-0.5 rounded-lg bg-white/[0.05] text-admin-muted/60">{c.style}</span>}</div>
          {c.subtitle && <p className="text-admin-champ/70 text-sm mt-1">{c.subtitle}</p>}
          {c.description && <p className="text-admin-muted/65 text-sm mt-2 max-w-2xl leading-relaxed">{c.description}</p>}
          <div className="flex flex-wrap gap-x-4 gap-y-1 mt-2 text-[11px] text-admin-muted/45">
            {c.curator && <span>Curadoria: {c.curator}</span>}
            {c.location && <span className="flex items-center gap-1"><Icon name="map" className="w-3.5 h-3.5" />{c.location}</span>}
            {c.budget_hint && <span className="flex items-center gap-1"><Icon name="tag" className="w-3.5 h-3.5" />{c.budget_hint}</span>}
          </div>
          {Array.isArray(c.tags) && c.tags.length > 0 && <div className="flex flex-wrap gap-1.5 mt-2">{c.tags.map((t, i) => <span key={i} className="text-[10px] px-2 py-0.5 rounded-md bg-white/[0.04] text-admin-muted/60">{t}</span>)}</div>}
        </div>
        <div className="flex items-center gap-2 shrink-0">
          <button onClick={share} className="flex items-center gap-2 glass-input text-admin-muted/70 hover:text-admin-champ px-4 py-2 rounded-xl text-sm transition-colors"><Icon name="share" className="w-4 h-4" />Compartilhar</button>
          {isMine && <button onClick={() => setAdding(true)} className="flex items-center gap-2 bg-admin-champ/12 hover:bg-admin-champ/20 text-admin-champ px-4 py-2 rounded-xl text-sm transition-colors"><Icon name="plus" className="w-4 h-4" />Adicionar item</button>}
          {isMine && <button onClick={() => setConfirmDel(true)} className="glass-input text-admin-muted/60 hover:text-admin-rose px-3 py-2 rounded-xl transition-colors" title="Excluir coleção"><Icon name="trash" className="w-4 h-4" /></button>}
        </div>
      </div>
      {confirmDel && (
        <div className="fixed inset-0 z-[95] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={() => setConfirmDel(false)}>
          <div className="glass-pop rounded-2xl p-6 w-full max-w-sm" onClick={(e) => e.stopPropagation()}>
            <h2 className="font-serif text-lg text-admin-text mb-2">Excluir coleção?</h2>
            <p className="text-admin-muted/60 text-sm mb-5">A coleção <span className="text-admin-text">"{c.title}"</span> e todos os itens serão removidos. Esta ação não pode ser desfeita.</p>
            <div className="flex justify-end gap-2"><button onClick={() => setConfirmDel(false)} className="px-4 py-2 rounded-xl text-sm text-admin-muted hover:text-admin-text">Cancelar</button><button onClick={() => { setConfirmDel(false); onDelete?.(c) }} className="px-4 py-2 rounded-xl text-sm bg-admin-rose/15 hover:bg-admin-rose/25 text-admin-rose">Excluir coleção</button></div>
          </div>
        </div>
      )}
